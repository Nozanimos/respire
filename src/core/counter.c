...
void counter_render(CounterState* counter, SDL_Renderer* renderer,
    ...
    // Flag static pour éviter de terminer dans le même frame où on atteint total_breaths
    static bool just_completed = false;
    ...
    //   - retention_type=0 (poumons pleins) : 10ème scale_min → scale_max → FIN
    //   - retention_type=1 (poumons vides) : 10ème scale_min → cycle complet → scale_min → FIN
    ...
    if (counter->waiting_for_scale_min && !just_completed) {
        if (counter->retention_type == 0) {
            ...
            if (is_at_max_now && !counter->was_at_max_last_frame) {
                counter->is_active = false;
                just_completed = false;  // Reset pour la prochaine session
    ...
    // Reset du flag just_completed à la frame suivante
    if (just_completed && !is_at_min_now) {
