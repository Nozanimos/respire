...
bool parser_widget_selector(cJSON* json_obj, LoaderContext* ctx, WidgetList* list) {
    ...
    cJSON* submenu_enabled = cJSON_GetObjectItem(json_obj, "submenu_enabled");

    // Validation des champs de base
    // En mode roller (submenu_enabled), options n'est pas obligatoire
    bool is_roller_mode = (submenu_enabled && cJSON_IsTrue(submenu_enabled));

    if (!cJSON_IsString(id) || !cJSON_IsString(nom_affichage) ||
        !cJSON_IsNumber(x) || !cJSON_IsNumber(y)) {
        debug_printf("❌ Widget selector invalide : champs de base manquants\n");
        return false;
    }

    if (!is_roller_mode && !cJSON_IsArray(options_array)) {
        debug_printf("❌ Widget selector invalide : options manquantes (mode classique)\n");
        return false;
        }
