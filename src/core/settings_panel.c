...
void retention_pattern_changed(int new_index) {
    ...
    debug_printf("✅ Pattern de rétention changé: %s (en attente de validation)\n",
...
void retention_roller_changed(int seq1_type, int seq1_count, int seq2_type, int seq2_count) {
    ...
    debug_printf("✅ Pattern roller modifié: %d×%s puis %d×%s (en attente de validation)\n",
...
void apply_button_clicked(void) {
    ...
    *ctx->main_config = ctx->panel->temp_config;
    ...
    if (ctx->hexagones && *ctx->hexagones && ctx->screen_width && ctx->screen_height) {
        ...
        debug_printf("✅ Hexagones réinitialisés (figés au centre, scale=1.0)\n");
    }
...
void recalculate_widget_layout(SettingsPanel* panel) {
    ...
    for (int g = 0; g < current_group; g++) {
        ...
        if (longest_widget) {
            container_width = longest_widget->local_roller_x + longest_widget->roller_width + 10;  // RIGHT_MARGIN (cohérent avec calculate_roller_x_offset)
        }
